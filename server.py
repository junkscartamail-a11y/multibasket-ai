...
Non inventare dati non visibili. timeRemaining è il tempo rimanente nel quarto."""
    r=client.chat.completions.create(model=OPENAI_MODEL,messages=[
...
def decide(ex,bankroll):
    home=ex.get("homeTeam") or "Squadra A"; away=ex.get("awayTeam") or "Squadra B"
    need=["homeScore","awayScore","quarter","timeRemaining","lineOU"]
    miss=[k for k in need if ex.get(k) is None]
    if miss:
        return nobet(home,away,"-","-","Dati insufficienti nello screenshot: devono essere leggibili punteggio, quarto, cronometro e linea bookmaker.",["Screenshot incompleto","Non faccio pronostici inventati","Puntata consigliata 0 €"])
    h=int(ex["homeScore"]); a=int(ex["awayScore"]); total=h+a
    q=int(ex["quarter"]); left=clock_min(ex["timeRemaining"]); line=float(ex["lineOU"])
    if left is None or q<1 or q>4 or left<0 or left>10:
        return nobet(home,away,f"{h}-{a}",f"{ex.get('timeRemaining')} Q{q}","Cronometro o quarto non affidabili: senza tempo corretto la stima sarebbe casuale.",["Tempo non validato","Rischio calcolo alto","Puntata consigliata 0 €"],line)
    played=(q-1)*10+(10-left); rem=max(0,40-played)
    ...
    if side and conf>=68 and st>0:
        ...
        reason=f"Stima screenshot-first: totale previsto {pred} contro linea {line}, valore {value:+.1f}. Ritmo {ritmo(ppm)}, giocati {played:.1f} minuti, restano {rem:.1f}. Stake prudente perché la fonte è lo screenshot."
        why=[f"Ritmo attuale: {ritmo(ppm)}",f"Scostamento dalla linea: {value:+.1f}",f"Totale previsto realistico: {pred}","Puntata ridotta perché non c'è feed live continuo"]
    elif abs(value)>=5:
        signal="OBSERVE"; action="OSSERVA"; text="Aspetta o rifai screenshot tra 1-2 minuti"
        reason=f"C'è valore teorico {value:+.1f}, ma affidabilità {conf}/100: non basta per giocare soldi veri."
        why=["Valore presente ma non solido","Mancano dati su falli, possessi e ritmo ultimi minuti","Puntata consigliata 0 €"]
    else:
        ...
        why=["Edge insufficiente","Rischio non compensato","Puntata consigliata 0 €"]
    return {"signal":signal,"action":action,"decision_text":text,"side":side,"line":line,"stake":st,"confidence":conf,"score":f"{h}-{a}","clock":f"{ex.get('timeRemaining')} Q{q}","teams":{"home":home,"away":away},"rhythm":ritmo(ppm),"total_predicted":pred,"final_score":f"{fh}-{fa}","value":round(value,1),"prob_over":po,"prob_under":pu,"reason":reason,"why":why,"source":"screenshot-first"}

def nobet(home,away,score,clock,reason,why,line="-"):
    return {"signal":"NO_BET","action":"NON GIOCARE","decision_text":"Non scommettere","side":None,"line":line,"stake":0,"confidence":0,"score":score,"clock":clock,"teams":{"home":home,"away":away},"rhythm":"non valutabile","total_predicted":"-","final_score":"-","value":0,"prob_over":50,"prob_under":50,"reason":reason,"why":why,"source":"screenshot-first"}

@app.route("/")
def home():return send_from_directory(".","index.html")

@app.route("/api/health")
def health():return jsonify({"ok":True,"telegram":bool(TG_TOKEN and TG_CHAT),"openai":bool(OPENAI_API_KEY),"mode":"screenshot-first"})
...
def shot():
    ...
    return jsonify({"extracted":ex,"decision":decide(ex,bankroll),"mode":"screenshot-first"})
...
def bet():
    ...
    tg(f"✅ <b>Giocata registrata</b>\\n{str(p.get('side')).upper()} {p.get('line')}\\nPuntata: {p.get('stake')} €\\nFonte: screenshot-first. Aggiorna con nuovo screenshot.")
    return jsonify({"ok":True,"message":"Giocata registrata. Per rivalutarla carica un nuovo screenshot."})

@app.route("/api/bet/quality")
def quality():return jsonify({"message":"In modalità screenshot-first la qualità si aggiorna caricando un nuovo screenshot."})

@app.route("/api/live/games")
def live_games():return jsonify({"count":0,"games":[],"message":"Modalità screenshot-first."})

@app.route("/api/live/analyze",methods=["POST"])
def live_analyze():return jsonify({"error":"Usa Analizza screenshot."}),400
